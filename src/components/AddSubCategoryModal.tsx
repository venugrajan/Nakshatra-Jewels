...
export default function AddSubCategoryModal({ categoryId, onClose }: AddSubCategoryModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      handleFirestoreError(err, OperationType.CREATE, 'subcategories');
    } finally {
      setLoading(false);
    }
  };
          ...
          >
            <div className="px-6 py-4 border-b border-high-border bg-slate-50 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-xs font-bold uppercase tracking-wider">New Sub-Category</h2>
              </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div className="space-y-1.5">
                <label className="data-label text-high-ink">Sub-Category Name</label>
                <input
                  autoFocus
                  type="text"
                  value={name}
                  onChange={(e) => setName(e.target.value)}
                  placeholder="e.g. Urban, Nature, Macro..."
